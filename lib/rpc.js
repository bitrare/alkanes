...
class AlkanesRpc extends base_rpc_1.BaseRpc {
    ...
    async holdersbytoken({ block, tx }, blockTag = "latest") {
        const payload = (0, outpoint_1.encodeHoldersByTokenInput)(block, tx);
        const response = await this._call({
            method: "alkanes_holdersbytoken",
            input: payload,
        }, blockTag);
        const decodedResponse = (0, outpoint_1.decodeHoldersByTokenResponse)(response);
        return decodedResponse;
    }
